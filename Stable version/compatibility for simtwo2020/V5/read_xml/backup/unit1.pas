...
var
  ...
  Doc: TXMLDocument;
  Robots: TDOMNodeList;
  Robot,x,y,priority,Direction,inode,steps,ActualSubMission,CounterSubMissions,SubMissions: TDOMNode;
  id_value,priority_value,x_value,y_value,dir_value,inode_value,steps_value,ActualSubMission_value,CounterSubMissions_value,TotalSubMissions_value,target_node_value,target_node_step_value,SubMissions_value:string;
  l1:integer;
  i,aux1:integer;
  SubMissions_a:array of integer;
begin
  ...
  begin
     ...
     id_value:=id.NodeValue;
     priority:=Node.FindNode('priority');
     priority_value:=priority.FirstChild.NodeValue;
     x:=Node.FindNode('x');
     x_value:=x.FirstChild.NodeValue;
     y:=Node.FindNode('y');
     y_value:=y.FirstChild.NodeValue;
     Direction:=Node.FindNode('Direction');
     dir_value:=Direction.FirstChild.NodeValue;
     inode:=Node.FindNode('node');
     inode_value:=inode.FirstChild.NodeValue;
     steps:=Node.FindNode('steps');
     steps_value:=steps.FirstChild.NodeValue;
     ActualSubMission:=Node.FindNode('ActualSubMission');
     ActualSubMission_value:=ActualSubMission.FirstChild.NodeValue;
     CounterSubMissions:=Node.FindNode('CounterSubMissions');
     CounterSubMissions_value:=CounterSubMissions.FirstChild.NodeValue;
     TotalSubMissions:=Node.FindNode('TotalSubMissions');
     TotalSubMissions_value:=TotalSubMissions.FirstChild.NodeValue;
     target_node:=Node.FindNode('target_node');
     target_node_value:=target_node.FirstChild.NodeValue;
     target_node_step:=Node.FindNode('target_node_step');
     target_node_step_value:=target_node_step.FirstChild.NodeValue;
     SubMissions:=Node.FindNode('SubMissions');
     SubMissions_value:=SubMissions.FirstChild.Attributes.Item[0];
     SubMissions_count:=SubMissions.FirstChild.FindNode('SubMission').ChildNodes.Count;
     setlength(SubMissions_a,SubMissions_count);
     for aux1:=0 to SubMissions_count-1 do
     begin
        SubMissions_a[aux1]:=strtoint(SubMissions.FirstChild.FindNode('SubMission').ChildNodes[aux1].NodeValue);
     end;
     form1.robots[aux1].id_robot:=strtoint(id_value);
     form1.robots[aux1].InitialIdPriority:=strtoint(priority_value);
     form1.robots[aux1].pos_X:=strtofloat(x_value);
     form1.robots[aux1].pos_y:=strtofloat(y_value);
     form1.robots[aux1].ipos_X:=form1.robots[aux1].pos_X;
     form1.robots[aux1].ipos_y:=form1.robots[aux1].pos_y;
     form1.robots[aux1].iDirection:=strtoint(dir_value);
     form1.robots[aux1].Direction:=form1.robots[aux1].Direction;
     form1.robots[aux1].inicial_node:=strtoint(inode_value);
     form1.robots[aux1].inicial_step:=strtoint(steps_value);
     form1.robots[aux1].ActualSubMission:=strtoint(ActualSubMission_value);
     form1.robots[aux1].CounterSubMissions:=strtoint(CounterSubMissions_value);
     form1.robots[aux1].TotalSubMissions:=strtoint(TotalSubMissions_value);
     form1.robots[aux1].target_node:=strtoint(target_node_value);
     form1.robots[aux1].target_node_step:=strtoint(target_node_step_value);
     form1.robots[aux1].SubMissions:=SubMissions_a;
  end;
   Links:= Doc.GetElementsByTagName('Link');
   for i:= 0 to Links.Count - 1 do
   begin
