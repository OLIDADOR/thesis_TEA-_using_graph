...
begin
 ...
 // s1:=length(CaminhosAgvs[i].coords);
        for aux2:=s to s1 do begin
          if ((check_array(unflawed_nodes,CaminhosAgvs[i].coords[aux2].node)=1) and (CaminhosAgvs[i].coords[aux2].node<>robot[i].curr_in_node)) then begin
          {s2:=length(r_aux);
...
begin
   ...
   if idf=0 then begin
   aux_id:=get_futhrest_exit_link_node(form1.map,form1.robots,i,CaminhosAgvs_af,current_step_f[i]); //pode ser mehlorado
   end else begin
    aux_id:=idf;
   end;
