...
begin
    ...
    if flagVelocities = true then begin
        ...
        while i<NUMBER_ROBOTS do begin
           ...
           MessageVelocities := MessageVelocities + 'N' + IntToStr( form1.robots[i].id_robot);
           MessageVelocities := MessageVelocities + 'P' + IntToStr( form1.robots[i].InitialIdPriority);
           MessageVelocities := MessageVelocities + 'T' + IntToStr( Ca[i]);
           MessageVelocities := MessageVelocities + 'S' + IntToStr(get_steps(CaminhosAgvs,i)-1);
           MessageVelocities1 := MessageVelocities1 + 'N' + IntToStr( form1.robots[i].id_robot);
           MessageVelocities1 := MessageVelocities1 + 'P' + IntToStr( form1.robots[i].InitialIdPriority);
           MessageVelocities1 := MessageVelocities1 + 'T' + IntToStr( Ca[i]);
           MessageVelocities1 := MessageVelocities1 + 'S' + IntToStr(get_steps(CaminhosAgvs,i)-1);
           l1:=length((CaminhosAgvs[i].coords));
           count:=0;
           ...
           count:=0;
           for aux1:=0 to l1-1 do begin
           if ((getXcoord(CaminhosAgvs[i].coords[aux1].node)<3) and (getYcoord(CaminhosAgvs[i].coords[aux1].node)<3)and (count<6)) then
           begin
           MessageVelocities1 := MessageVelocities1 + 'I' + IntToStr(aux1)
                                                  + 'X' + FloatToStr(round2(getXcoord(CaminhosAgvs[i].coords[aux1].node),3))
                                                  + 'Y' + FloatToStr(round2(getYcoord(CaminhosAgvs[i].coords[aux1].node),3))
                                                  + 'D' + IntToStr(CaminhosAgvs[i].coords[aux1].direction);
           count:=count+1;
