...
var
 s1,aux1,aux2,c_a:integer;
 r_aux:array of integer;
begin
  s1:=length(CaminhosAgvs[i].coords);
  for aux1:=0 to s1-1 do begin
     if CaminhosAgvs[i].coords[aux1].node=n then begin
        setlength(r_aux,s1-(aux1+1));
        c_a:=0;
...
function save_flaw_location(CaminhosAgvs:Caminhos; agvs:r_node; i:integer; nodelist:a_node):Coms_flaws_per_robot;
var
    at,aux1,aux2,aux_id,s1,s2:integer;
...
begin
   ...
   aux_id:=get_linked_node(nodelist,agvs[i].pos_X,agvs[i].pos_y,1,CaminhosAgvs[i].coords[1].node);
...
var
 s1,aux1,aux2,c_a,c1,c2,diff:integer;
 r_aux:array of integer;
begin
  s1:=length(CaminhosAgvs[i].coords);
  for aux1:=0 to s1-1 do begin
...
begin
   ...
   aux_robots_flaws.out_node[s1]:=get_linked_node(form1.full_nodelist,agvs[i].pos_X,agvs[i].pos_y,1,CaminhosAgvs[i].coords[1].node);
   aux_robots_flaws.detected_nodes[s1]:=update_flaw_path(aux_robots_flaws.out_node[s1],aux_robots_flaws.in_node[s1],i,CaminhosAgvs);
        ...
        if ((flagVelocities = true) and (data[1] = 'R')) then begin
          ...
          while i<NUMBER_ROBOTS do begin
              ...
              //coms flaw no communication with robot
               robots_flaws[i]:=save_flaw_location(CaminhosAgvs,form1.robots,i,form1.full_nodelist);
               timestamp_coms[i]:=total_seconds;
              end else begin
              ...
              if ((step_complete[i]=0) or  (type_of_movement[i]=1)) then begin
                  ...
                  if check_if_still_on_path(CaminhosAgvs,Form1.robots,i,0.005,current_step)=1 then
                  begin
                  Edit4.Text:='TEA';
                  followLine[i]:=false;
                  followCircle[i]:=false;
                  ...
                  end else if check_if_still_on_path(CaminhosAgvs,Form1.robots,i,0.005,current_step)=0 then
                  begin
