...
function checkifflawnode(id:integer):integer;
var
  aux1,aux2,aux3,at,s1,s2,s3:integer;
begin
   s1:=length(controlo.robots_flaws);
   at:=0;
   if s1>0 then begin
   for aux1:=0 to s1-1 do begin
      s2:=length(controlo.robots_flaws[aux1].detected_nodes);
      if s2>0 then begin
      for aux2:=0 to s2-1 do begin
          s3:=length(controlo.robots_flaws[aux1].detected_nodes[aux2]);
         for aux3:=0 to s3-1 do begin
           if controlo.robots_flaws[aux1].detected_nodes[aux2][aux3]=id then begin
            at:=0;
           end;
         end;
      end;
...
begin
  ...
  if l1>0 then begin
   //Print Links
    ...
    for aux1:=0 to l1-1 do
        begin
          X_print:=round(form1.full_nodelist[aux1].pos_X*200);
          Y_print:=round(form1.full_nodelist[aux1].pos_y*200);
              ...
              id_l:=form1.full_nodelist[aux1].links[aux2].node_to_link;
               for aux3:=0 to l1-1 do
                   begin
                     ...
                     begin
                        X_print1:=round(form1.full_nodelist[aux3].pos_X*200);
                        Y_print1:=round(form1.full_nodelist[aux3].pos_y*200);
  ...
  //Print Nodes
   ...
   for aux1:=0 to l3-1 do
       begin
          ...
          X_print:=round(form1.full_nodelist[aux1].pos_X*200);
          Y_print:=round(form1.full_nodelist[aux1].pos_y*200);
          if checkifflawnode(id_i)=1 then
          begin
             Canvas.Pen.Color:=clRed;
          end
          else if checkifflawnode(id_i)=0 then
          begin
