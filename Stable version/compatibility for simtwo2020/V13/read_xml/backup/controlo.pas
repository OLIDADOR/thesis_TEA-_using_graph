  ...
  id2:=0;
   ...
   begin
     ...
     if n_curr=id1 then
        ...
        x1:=round_with_th(x1,x,0.1);
        y1:=nodelist[aux1].pos_Y;
        y1:=round_with_th(y1,y,0.1);
        diff1x:=(x-x1);
        diff1y:=(y-y1);
...
begin
    ...
    if ((flagVelocities = true) and (b_pathsend<>1)) then begin
        ...
        if ind_robo<NUMBER_ROBOTS then begin
           ...
           if ((getXcoord(CaminhosAgvs[ind_robo].coords[aux1].node)<3) and (getYcoord(CaminhosAgvs[ind_robo].coords[aux1].node)<3)and (count<15)) then
           begin
