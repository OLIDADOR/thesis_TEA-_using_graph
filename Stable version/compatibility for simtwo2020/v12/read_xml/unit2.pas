...
function checkifflawnode(id:integer; flaws:controlo.cf_robots):integer;
var
...
begin
   ...
   for aux1:=0 to s1-1 do begin
      s2:=length(flaws[aux1].detected_nodes);
      if s2>0 then begin
      for aux2:=0 to s2-1 do begin
          s3:=length(flaws[aux1].detected_nodes[aux2]);
         for aux3:=0 to s3-1 do begin
           if flaws[aux1].detected_nodes[aux2][aux3]=id then begin
            at:=1;
           end;
         end;
      end;
...
var
  ...
  aux_flaw:controlo.cf_robots;
begin
  aux_flaw:=controlo.robots_flaws;
