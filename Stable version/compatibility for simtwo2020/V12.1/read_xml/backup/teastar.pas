...
procedure save_path_with_flaws_as_obstacle(var Map: TAStarMap; var CaminhosAgvs_flaw:Caminhos; var s_td:integer; id:integer;coms_flaws:controlo.cf_robots);
var
 aux1,aux2,a1,id1,se1,a2,id2,se2,s1,s2,s3,s4,f_noexit,diff:integer;
 aux_id,aux_s,r,aux_c:integer;
begin
   s1:=length(CaminhosAgvs_flaw[id].coords);
   a1:=99999;
   for aux1:=0 to s1-1 do begin
         if CaminhosAgvs_flaw[id].coords[aux1].node=coms_flaws[id].curr_in_node then begin
              a1:=aux1;
              id1:=CaminhosAgvs_flaw[id].coords[aux1].node;
              se1:=CaminhosAgvs_flaw[id].coords[aux1].steps;
              break;
         end;
   end;
   r:=0;
   aux_c:=0;
    ...
    if ((r<>0) and (aux_c<>0))  then begin
     //a falha ja foi mapeada
     s4:=length(coms_flaws[r].detected_nodes[aux_c]);
     end else begin
     //não sabemos o tamanho da falha ainda  por isso reservamos os nós até encontrar um nó já mapeado (not flawed)
     a2:=CaminhosAgvs_flaw[id].steps;
     diff:=se2-se1;
     for aux1:=a1 to a2 do begin
       aux_id:=CaminhosAgvs_flaw[id].coords[aux1].node;
     for aux2:=0 to diff+s_td do begin
     ...
     a2:=find_possible_out_node(CaminhosAgvs_flaw,a1,id,coms_flaws);
     if a2<>99999 then begin
     f_noexit:=0;
     if a2>a1 then begin
     id2:=CaminhosAgvs_flaw[id].coords[a2].node;
     se2:=CaminhosAgvs_flaw[id].coords[a2].steps;
     end else begin
        //get next occour of node
        id2:=CaminhosAgvs_flaw[id].coords[aux2].node;
        se2:=CaminhosAgvs_flaw[id].coords[aux2].steps;
        for aux1:=0 to s1-1 do begin
         if CaminhosAgvs_flaw[id].coords[aux1].node=id2 then begin
              a2:=aux1;
              break;
         end;
   end;
     ...
     f_noexit:=1;
     for aux1:=0 to s1-1 do begin
         if check_if_nodeexist(CaminhosAgvs_flaw[id].coords[aux1].node)=0 then begin
              a2:=aux1-1;
              id2:=CaminhosAgvs_flaw[id].coords[aux1-1].node;
              se2:=CaminhosAgvs_flaw[id].coords[aux1-1].steps;
              break;
         end;
     end;
end;
    end;
if a2>CaminhosAgvs_flaw[id].steps then begin
   a2:=CaminhosAgvs_flaw[id].steps;
   id2:=CaminhosAgvs_flaw[id].coords[a2].node;
   se2:=CaminhosAgvs_flaw[id].coords[a2].steps;
end;
...
if f_noexit=1 then begin
     for aux2:=0 to NUM_LAYERS-1 do begin
      Map.GraphState[id2-1][aux2]:=OBSTACLEROBOTINIT;
     end;
end;
end;
...
begin
  ...
  if aux2=1 then begin
       for aux1:=0 to (NUMBER_ROBOTS-1) do begin
          if  coms_flaws[aux1].isactive=1 then begin
               td:=5;
               save_path_with_flaws_as_obstacle(map, CaminhosAgvs_flaw,td,aux1,coms_flaws);
          end;
       end;
  end;
