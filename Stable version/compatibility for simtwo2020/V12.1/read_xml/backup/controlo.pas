...
function save_flaw_location(CaminhosAgvs:Caminhos; agvs:r_node; i:integer; nodelist:a_node; s:integer;robots_flaws:cf_robots):Coms_flaws_per_robot;
var
...
begin
   ...
   aux_robots_flaws.active_consecutive_hits:=1;
   s4:=length(nodelist);
   idf:=0;
   ...
   at:=0;
   aux_robots_flaws.curr_in_node:=aux_id;
   if s1>0 then begin
   ...
   for aux1:=0 to s2-1 do begin
       if aux_robots_flaws.out_node[aux1]=aux_id then begin
         at:=1;
         aux_robots_flaws.flaw_ind:=aux1;
       end;
   end;
...
function save_exit_node(agvs:r_node;i:integer;robots_flaws:cf_robots):Coms_flaws_per_robot;
var
    aux_robots_flaws: Coms_flaws_per_robot;
...
begin
   ...
   aux_robots_flaws.curr_out_node:=aux_id;
...
begin
     ...
     for aux1:=0 to s1-1 do begin
        ...
        if ((CaminhosAgvs[i].coords[aux1].node=n) and(aux1>=c1)) then begin
          c2:=aux1;
        end;
     end;
     diff:=c2-c1;
     setlength(r_aux,s2+diff+1);
     c_a:=s2-1;
     for aux2:=c1 to c2 do begin
        r_aux[c_a]:= CaminhosAgvs[i].coords[aux2].node;
        c_a:=c_a+1;
...
begin
   ...
   for aux1:=a1+1 to a2-1 do begin
...
var
    ...
    s1,cont,cont2,aux_step_complete,s4,n_curr,aux1,idf:integer;
        ...
        if ((flagVelocities = true) and (data[1] = 'R')) then begin
          ...
          for aux1:=0 to s4-1 do
           ...
           if robots_flaws[id_rob-1].isdetecting=1 then begin
              robots_flaws[id_rob-1].isdetecting:=0;
                robots_flaws[id_rob-1]:=save_exit_node(form1.robots,id_rob-1,robots_flaws);
          ...
          while i<NUMBER_ROBOTS do begin
              ...
              if robots_flaws[i].isactive=0 then begin
               robots_flaws[i]:=save_flaw_location(CaminhosAgvs,form1.robots,i,form1.full_nodelist,current_step[i],robots_flaws);
               f_replan:=1;
               end else begin
