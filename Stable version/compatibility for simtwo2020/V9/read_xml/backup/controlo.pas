...
var
    at,aux1,aux2,aux_id,s1:integer;
...
begin
   ...
   if at=0 then begin
      setlength(aux_robots_flaws.in_node, l1+1);
      aux_robots_flaws.in_node[l1]:=aux_id;
    ...
    if (data <> '') then begin
        if ((flagVelocities = true) and (data[1] = 'R')) then begin
          ...
          while i<NUMBER_ROBOTS do begin
              ...
              end else if ((timestamp_coms[i]+1<=total_seconds) and (type_of_movement[i]<>0) and (timestamp_coms[i]<>0))  then begin
