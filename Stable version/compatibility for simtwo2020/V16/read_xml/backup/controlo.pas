...
function  get_futhrest_link_node(map:TAStarMap;agvs:r_node;id:integer;CaminhosAgvs:Caminhos;s:integer):integer;
var
    ...
    for aux1:=0 to s1-1 do begin
       if map.TEA_GRAPH[aux1][0].id=nid then begin
          ...
          for aux2:=0 to s2-1 do begin
            ...
            if CaminhosAgvs[id].coords[s].node<>lid then begin
                r:=lid;
                break;
            end;
          end;
...
function get_futhrest_exit_link_node(map:TAStarMap;agvs:r_node;id:integer;CaminhosAgvs:Caminhos;se:integer):integer;
var
...
begin
 ...
 function check_unflawed_node(id:integer;unflawed_nodes:i_array):i_array;
 var
 s1,aux1:integer;
 aux_r:i_array;
 begin
   ...
   aux_r:=unflawed_nodes;
   for aux1:=0 to s1-1 do begin
       if unflawed_nodes[aux1]=id then begin
...
begin
 ...
 // s1:=length(CaminhosAgvs[i].coords);
  ...
  if at=0 then begin
       ...
       setlength(r_aux,s2+1);
       r_aux[s2]:= n;
       if n=robot[i].curr_in_node then begin
        i_end[i]:=1;
        end;
  end;
  get_remaining_path_nodes:=r_aux;
end;
function update_flaw_in_node(CaminhosAgvs:Caminhos; i:integer;n:integer;unflawed_nodes:i_array;d_node:i_array; robots:cf_robots; s:integer):i_array;
var
   s1,s2,aux1:integer;
  a_ar,d_ar:i_array;
begin
...
begin
   ...
   //aux_id:=get_linked_node(nodelist,agvs[i].pos_X,agvs[i].pos_y,1,CaminhosAgvs[i].coords[s].node);
    aux_id:=get_futhrest_link_node(form1.map,form1.robots,i,CaminhosAgvs,s);
   end else begin
    aux_id:=idf;
   end;
   ...
   if at=0 then begin
      ...
      unflawed_nodes:=check_unflawed_node(aux_id,unflawed_nodes);
      robots_flaw_status[i].flaw_ind:=s1;
         ...
         aux_robots_flaws.detected_nodes[ind]:=update_flaw_in_node(CaminhosAgvs,i,aux_id,unflawed_nodes, aux_robots_flaws.detected_nodes[ind],robots_flaw_status,s);
         unflawed_nodes:=check_unflawed_node(aux_id,unflawed_nodes);
...
begin
   ...
   if idf=0 then begin
   aux_id:=get_futhrest_exit_link_node(form1.map,form1.robots,i,CaminhosAgvs_af,current_step_f[i]);
   end else begin
    aux_id:=idf;
   end;
...
begin
    ...
    for aux1:=0 to s1-1 do begin
     ...
     for aux1:=0 to s1-1 do begin
       diff:=s[aux1]-min;
       if diff>0 then begin
           x_n:=getXcoord(CaminhosAgvs[aux1].coords[s[aux1]].node);
           y_n:=getYcoord(CaminhosAgvs[aux1].coords[s[aux1]].node);
           x_r:=agvs[aux1].pos_X;
           y_r:=agvs[aux1].pos_y;
           diffx:=abs(x_n-x_r);
           diffy:=abs(y_n-y_r);
           difft:=diffx+diffy;
           if difft<=0.3 then begin
             r:=1;
           end;
       end;
     end;
...
var
lnode,lonode,inode,onode,aux1,s1,s2,s3,aux2,aux3,aux4,aux5,aux6,aux7,r:integer;
begin
  r:=0;
  for aux1:=0 to NUMBER_ROBOTS-1 do begin
   ...
   for aux2:=0 to NUMBER_ROBOTS-1 do begin
    ...
    if ((onode=inode) and (aux1<>aux2)) then begin
    r:=1;
    end;
        ...
        if ((flagVelocities = true) and (data[1] = 'R')) then begin
         ...
         // ttttt:=unflawed_node[0];
          ...
          for aux1:=0 to s4-1 do
           ...
           aux_exit_flaw_node:=get_futhrest_exit_link_node(form1.map,form1.robots,id_rob-1,CaminhosAgvs_af,current_step_f[id_rob-1]);
           end else begin
