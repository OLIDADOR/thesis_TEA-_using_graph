...
type
  ...
  Coordinates = record
   ...
   coms_flaw_location =record
      ...
      out_node:array of  array of integer;
...
function  get_futhrest_link_node(map:TAStarMap;agvs:r_node;id:integer):integer;
var
      ...
      begin
        if form1.map.TEA_GRAPH[aux1][0].id=i then begin
            for aux2:=aux1 to length(form1.map.TEA_GRAPH)-1 do begin
              ...
              setlength(form1.map.GraphState, length(form1.map.TEA_GRAPH)-1,NUM_LAYERS);
              setlength(form1.map.HeapArray.data, (length(form1.map.TEA_GRAPH)-1)*NUM_LAYERS);
              //form1.map:=map_aux;
        end;
      end;
 end;
...
function get_remaining_path_nodes(CaminhosAgvs:Caminhos; i:integer;n:integer;unflawed_nodes:i_array;robot:cf_robots):i_array;
var
...
begin
 ...
 // s1:=length(CaminhosAgvs[i].coords);
  for aux1:=0 to s1 do begin
     if ((CaminhosAgvs[i].coords[aux1].node=n) and (n<>0)) then begin
        for aux2:=aux1 to s1 do begin
          if ((check_array(unflawed_nodes,CaminhosAgvs[i].coords[aux2].node)=1) and (CaminhosAgvs[i].coords[aux2].node<>robot[i].curr_in_node) and (at=0)) then begin
         break;
...
function update_flaw_in_node(CaminhosAgvs:Caminhos; i:integer;n:integer;unflawed_nodes:i_array;d_node:i_array; robots:cf_robots):i_array;
var
   s1,s2,aux1:integer;
  a_ar,d_ar:i_array;
begin
   a_ar:=get_remaining_path_nodes(CaminhosAgvs,i,n,unflawed_nodes,robots);
   d_ar:=d_node;
   s1:=length(a_ar);
   for aux1:=0 to s1-1 do begin
    if check_array(d_ar,a_ar[aux1])=1 then begin
    end else begin
        s2:=length(d_ar);
        setlength(d_ar,s2+1);
        d_ar[s2]:=a_ar[aux1]
...
var
r,s1,s2,s3,aux3,aux1,aux2,c_obj,c:integer;
begin
  r:=0;
...
function save_flaw_location(CaminhosAgvs:Caminhos; agvs:r_node; i:integer; nodelist:a_node; s:integer;robots_flaws_l:coms_flaw_location;robots_flaw_status:cf_robots;unflawed_nodes:i_array):coms_flaw_location;
var
...
begin
   ...
   //aux_id:=get_linked_node(nodelist,agvs[i].pos_X,agvs[i].pos_y,1,CaminhosAgvs[i].coords[s].node);
    aux_id:=get_futhrest_link_node(form1.map,form1.robots,i);
   end else begin
    aux_id:=idf;
   end;
   ...
   if at=0 then begin
      aux_i:=get_remaining_path_nodes(CaminhosAgvs,i,aux_id,unflawed_nodes,robots_flaw_status);
      if check_if_c_nodes_already_detected(aux_i,robots_flaws_l,2)=0 then begin
      ...
      aux_robots_flaws.detected_nodes[s1]:=get_remaining_path_nodes(CaminhosAgvs,i,aux_id,unflawed_nodes,robots_flaw_status);
      robots_flaw_status[i].flaw_ind:=s1;
         ...
         aux_robots_flaws.detected_nodes[ind]:=update_flaw_in_node(CaminhosAgvs,i,aux_id,unflawed_nodes, aux_robots_flaws.detected_nodes[ind],robots_flaw_status);
...
function save_exit_node(agvs:r_node;i:integer;robots_flaws:cf_robots;coms_f_l:coms_flaw_location):coms_flaw_location;
var
    aux_robots_flaws: coms_flaw_location;
...
begin
   ...
   aux_robots_flaws.out_node[s1][0]:=aux_id;
...
function update_afected_nodes(coms_f_l:coms_flaw_location;robots_flaws:cf_robots;i:integer):coms_flaw_location;
var
    s1,s2,aux1,aux2,c_a,c1,c2,diff:integer;
    r_aux:coms_flaw_location;
begin
     r_aux:=coms_f_l;
     s1:=length(r_aux.out_node[robots_flaws[i].flaw_ind]);
     setlength(r_aux.out_node[robots_flaws[i].flaw_ind],s1+1);
     r_aux.out_node[robots_flaws[i].flaw_ind][s1]:=robots_flaws[i].curr_out_node;
...
procedure Update_missions_in_flaw(var agvs:R_NODE;CaminhosAgvs_flaw:Caminhos;r:integer;robots_flaws:cf_robots);
var
...
begin
   ...
   for aux1:=0 to s1-1 do begin
         if ((CaminhosAgvs_flaw[r].coords[aux1].node=robots_flaws[r].curr_out_node) and (aux1<>a1) and (aux1<>a1+1)) then begin
              a2:=aux1;
              id2:=CaminhosAgvs_flaw[r].coords[aux1].node;
              se2:=CaminhosAgvs_flaw[r].coords[aux1].steps;
              break;
         end;
   end;
   end else begin
...
begin
    ...
    max:=0;
    for aux1:=0 to s1-1 do begin
       if a_i[aux1]>=max then begin
          max:=a_i[aux1];
       end;
        if a_i[aux1]<=min then begin
          min:=a_i[aux1];
       end;
    end;
    diff:=max-min;
        ...
        if ((flagVelocities = true) and (data[1] = 'R')) then begin
         ...
         // ttttt:=unflawed_node[0];
          ...
          for aux1:=0 to s4-1 do
           ...
           aux_exit_flaw_node:=get_futhrest_link_node(form1.map,form1.robots,id_rob-1);
           end else begin
           aux_exit_flaw_node:=idf;
           end;
           robots_flaws[id_rob-1].curr_out_node:=aux_exit_flaw_node;
           Update_missions_in_flaw(form1.robots,CaminhosAgvs_af,id_rob-1,robots_flaws);
           f_replan:=1;
           if robots_flaws[id_rob-1].isdetecting=1 then begin
              robots_flaws[id_rob-1].isdetecting:=0;
              flaw_location:=save_exit_node(form1.robots,id_rob-1,robots_flaws,flaw_location);
                //robots_flaws[id_rob-1].detected_nodes:=update_flaw_path(form1.robots,id_rob-1,CaminhosAgvs);
             end else begin
                 if check_array(flaw_location.out_node[robots_flaws[id_rob-1].flaw_ind],aux_exit_flaw_node)=1 then begin
                end else begin
                  flaw_location:=update_afected_nodes(flaw_location,robots_flaws,id_rob-1);
                end;
                end;
          ...
          while i<NUMBER_ROBOTS do begin
              ...
              if robots_flaws[i].isactive=0 then begin
               flaw_location:=save_flaw_location(CaminhosAgvs,form1.robots,i,form1.full_nodelist,current_step[i],flaw_location,robots_flaws,unflawed_node);
               //robots_flaws[i].isactive:=1;
               //ttttt:=length(flaw_location.detected_nodes[0]);
               f_replan:=1;
               end else begin
              ...
              {if ((step_complete[i]=0) or  (type_of_movement[i]=1)) then begin
               ...
               if ((check_if_still_on_path(CaminhosAgvs,Form1.robots,i,0.05,current_step)=1) or (f_replan=1) or (max_diff_step(current_step)>1) {or (checkforflaws(robots_flaws)=1)}) then begin
                    ...
                    if ((step_complete[i]=0) and  (type_of_movement[i]<>1)) then begin
                        Edit4.Text:='MOVE';
                         ...
                         for aux1:=0 to NUMBER_ROBOTS-1 do begin
                          ...
                          if ((step_complete[aux1]=1) or (current_step[aux1]>current_step[i])) then begin
                             cont2:=cont2+1;
                          end;
                      end;
...
begin
    ...
    if ((flagVelocities = true) and (b_pathsend<>1)) then begin
        ...
        if ind_robo<NUMBER_ROBOTS then begin
           ...
           if ((getXcoord(CaminhosAgvs[ind_robo].coords[aux1].node)<3) and (getYcoord(CaminhosAgvs[ind_robo].coords[aux1].node)<3) and (count<15) and (CaminhosAgvs[ind_robo].coords[aux1].node<>0)) then
           begin
